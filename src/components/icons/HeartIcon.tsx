...
export const HeartIcon = ({ isActive, hasLiked, type, size }: Props) => {
  const isFilled = type === 'notification' ? isActive : hasLiked;
  const fillColor = type === 'like' && hasLiked ? '#ff3040' : 'currentColor';
  const label = type === 'like' ? 'Like' : 'Notifications';
